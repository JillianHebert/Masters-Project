...
sim_data <- function(alpha = 0.05, btwn_sub, wthn_sub, n_sub, n_rep = 20){
  ...
  for(j in 1:rep) {
    ...
    trt_means <- c(10, 10) #same number --> reject 5% of the time
    ...
    e_sub_j <- rep(scale(rgamma(n = n_sub * n_trt, shape = eta)) * sig_sub, each = n_rep)
    mu_trt_i <- rep(trt_means, each = n_sub * n_rep)
    
    #Construct the response variable by summing components, errors and trt effects
    sim.df <- cbind.data.frame(trt_i, sub_j, rep_k, mu_trt_i, e_sub_j, e_k)
    sim.df <- sim.df %>% mutate(Y = mu_trt_i + e_sub_j + e_k)
    
    #Create a version of the df that averages (both mean and median) across reps within subjects
    sim.df.group <- sim.df %>% group_by(sub_j, trt_i) %>% 
      dplyr::summarise(Y_pooled = mean(Y), Y_median = median(Y), .groups = "keep")
    ...
    #Fit the fixed model and store the associate treatment p-value (subject as block)
    ...
    un_mod <- lme(Y ~ trt_i, random = list (~1 | sub_j), data = sim.df, method = "ML")
    ...
    rest_mod <- lme(Y ~ trt_i, random = list(~ 1 | sub_j), data = sim.df, method = "REML")
    ...
    #Assemble p-values and CI from all models of interest and bind them with previous sims
    table <- as.data.frame(cbind(pool_p, fixed_p, un_p, rest_p))
    final <- rbind(final, table)
    
    #Assemble confidence intervals from all models and bind them with previous sims
    ci_table <- as.data.frame(cbind(pool_lower_ci, pool_upper_ci, fixed_lower_ci, fixed_upper_ci, un_lower_ci, un_upper_ci, rest_lower_ci, rest_upper_ci))
  ...
  #Create power table
  power_table <- final %>% dplyr::summarise(pool_power = sum(pool_p < 0.05) / rep,
  ...
  #Create CI table
  average_ci_table <- final_ci_table %>% dplyr::summarise(avg_pool_lower = mean(pool_lower_ci),
...
n_cases <- dim(cases)[1]
results <- as.data.frame(cbind(case = seq(1:n_cases), pool_power = NA, fixed_power = NA, un_power = NA, rest_power = NA, avg_pool_lower = NA, avg_pool_upper = NA, avg_fixed_lower = NA, avg_fixed_upper = NA, avg_un_lower = NA, avg_un_upper = NA, avg_rest_lower = NA, avg_rest_upper = NA))

#Loop through all 9 cases and save results
for(i in 1:n_cases){
  args <- as.numeric(cases[i, 1:5])
  results[i, 2:13] <- sim_data(alpha = args[1], btwn_sub = args[2], wthn_sub = args[3], 
                               n_sub = args[4], n_rep = args[5])
}

#Format output
equal_sim_results <- cbind(cases[, 1:5], results[, -1])
equal_sim_results <- as.data.frame(equal_sim_results)
names(equal_sim_results) <- c("Alpha", "Between Subject SD", "Within Subject SD", "Number of Subjects", "Number of Repetitions", "Pooled Power", "Fixed Power", "ML Power", "REML Power", "Pooled Lower CI", "Pooled Upper CI", "Fixed Lower CI", "Fixed Upper CI", "ML Lower CI", "ML Upper CI", "REML Lower CI", "REML Upper CI")
...
equal_data <- equal_sim_results %>%
  select(`Pooled Upper CI`, `Pooled Lower CI`, `Fixed Upper CI`,`Fixed Lower CI`,
         `ML Upper CI`, `ML Lower CI`, `REML Upper CI`, `REML Lower CI`) %>%
...
#Separate by model
pooled_data <- equal_data %>% dplyr::summarise(term = "Pooled",
                                               pool_est = pool_estimate,
                                               pool_low = `Pooled Lower CI`,
                                               pool_up = `Pooled Upper CI`,
                                               .groups = "keep") %>% t()

fixed_data <- equal_data %>% dplyr::summarise(term = "Fixed",
                                              fixed_est = fixed_estimate,
                                              fixed_low = `Fixed Lower CI`,
                                              fixed_up = `Fixed Upper CI`,
                                              .groups = "keep") %>% t()

ml_data <- equal_data %>% dplyr::summarise(term = "ML",
                                           ml_est = ml_estimate,
                                           ml_low = `ML Lower CI`,
                                           ml_up = `ML Upper CI`,
                                           .groups = "keep") %>% t()

reml_data <- equal_data %>% dplyr::summarise(term = "REML",
...
for(i in 1:9) {
  ...
  case <- as.data.frame(rbind(pooled_data[, i], fixed_data[, i], ml_data[, i], reml_data[, i]))
...
#Format output
unequal_sim_results <- cbind(cases[, 1:5], results[, -1])
unequal_sim_results <- as.data.frame(unequal_sim_results)
names(unequal_sim_results) <- c("Alpha", "Between Subject SD", "Within Subject SD", "Number of Subjects", "Number of Repetitions", "Pooled Power", "Fixed Power", "ML Power", "REML Power", "Pooled Lower CI", "Pooled Upper CI", "Fixed Lower CI", "Fixed Upper CI", "ML Lower CI", "ML Upper CI", "REML Lower CI", "REML Upper CI")
...
pooled_data <- unequal_data %>% dplyr::summarise(term = "Pooled",
                                                 pool_est = pool_estimate,
                                                 pool_low = `Pooled Lower CI`,
                                                 pool_up = `Pooled Upper CI`,
                                                 .groups = "keep") %>% t()

fixed_data <- unequal_data %>% dplyr::summarise(term = "Fixed",
                                                fixed_est = fixed_estimate,
                                                fixed_low = `Fixed Lower CI`,
                                                fixed_up = `Fixed Upper CI`,
                                                .groups = "keep") %>% t()

ml_data <- unequal_data %>% dplyr::summarise(term = "ML",
                                             ml_est = ml_estimate,
                                             ml_low = `ML Lower CI`,
                                             ml_up = `ML Upper CI`,
                                             .groups = "keep") %>% t()

reml_data <- unequal_data %>% dplyr::summarise(term = "REML",
                                               reml_est = reml_estimate,
                                               reml_low = `REML Lower CI`,
                                               reml_up = `REML Upper CI`,
                                               .groups = "keep") %>% t()
...
for(i in 1:9) {
  ...
  graph <- ggplot(aes(x = Term, y = Estimate), data = case) +
    ...
    geom_linerange(aes(ymin = `Lower CI`, ymax = `Upper CI`)) +
    coord_flip() +
    geom_hline(yintercept = 3, color = "red") + #Change center to difference (10, 10 * (0.5 + gam_sd))
    theme_bw() +
    theme(strip.background = element_blank())
